...
def solution(n, build_frame):
    ...
    for frame in build_frame:
        ...
        if condition == [0, 1] and pos[1] < (n - 1):
            print(f"기둥 설치 조건!!")
            if pos[1] == 0 or surrounding[1][2][1] == 1 or surrounding[3][2][1] == 1 or surrounding[2][1][0] == 1:
                print(f"기둥 설치 O")
        ...
        # (1) 위에 기둥이 있는데 지지해줄 보가 없을 때, (2) 위에 보가 있는데 혼자 남을 때 (3) 왼쪽 위에 보가 있는데 혼자 남을 때는 삭제하지 못한다.
            ...
            print(f"기둥 삭제 조건!!")
            if not ((surrounding[2][3][0] == 1 and surrounding[2][3][1] == 0 and surrounding[1][3][1] == 0) or
                    (surrounding[2][3][1] == 1 and surrounding[3][3][1] == 0 and surrounding[3][2][0] == 0) or
                    (surrounding[1][3][1] == 1 and surrounding[0][3][1] == 0 and surrounding[1][2][0] == 0)):
                print(f"기둥 삭제 O")
                structure[pos[0]][pos[1]][0] = 0
            else:
                print(f"기둥 삭제 X")
        # 3. 보 설치 조건
        # 설치할 보가 (1) 기둥 위에 있거나, (2) 오른쪽 아래 좌표가 기둥 위에 있거나, (3, 4) 양쪽에 보가 있을 때
        elif condition == [1, 1] and pos[0] < (n - 1):
            print(f"보 설치 조건!!")
            if surrounding[2][1][0] == 1 or surrounding[3][1][0] == 1 or (surrounding[1][2][1] == 1 and surrounding[3][2][1] == 1):
                ...
                print(f"보 설치 X")
        # 4. 보 삭제 조건
        # (1) 위에 기둥이 있는데 지지해줄 보가 없을 때, (2) 오른쪽 위에 기둥이 있는데 지지해줄 보가 없을 때,
        # (3) 왼쪽에 보가 있는데 지지해줄 기둥이 없을 때, (4) 오른쪽에 보가 있는데 지지해줄 기둥이 없을 때는 삭제하지 못한다.
        elif condition == [1, 0]:
            print(f"보 삭제 조건!!")
            if not ((surrounding[2][2][0] == 1 and surrounding[1][2][1] == 0) or
                    (surrounding[3][2][0] == 1 and surrounding[3][4][1] == 0) or
                    (surrounding[1][2][1] == 1 and surrounding[1][1][0] == 0) or
                    (surrounding[3][2][1] == 1 and surrounding[4][1][0] == 0)):
                print(f"보를 삭제 O")
