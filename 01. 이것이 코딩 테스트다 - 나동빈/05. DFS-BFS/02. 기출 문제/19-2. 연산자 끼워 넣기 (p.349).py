...
# 사용할 수 있는 연산자를 확인하는 함수
def availabe_operators(oper_list):
    next_oper = [False, False, False, False]
    for i in range(4):
        if oper_list[i] > 0:
            next_oper[i] = True
    return next_oper
...
def solution(depth, res, oper_list):
    ...
    for i in range(4):
        if oper_list[i]:
            ...
            operators[i] -= 1
            solution(depth + 1, res, operators)
...
answer = []
solution(1, 0, availabe_operators(operators))
